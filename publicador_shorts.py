...
YT_TOKEN_JSON = os.environ.get("YOUTUBE_TOKEN_IT_SHORTS")
HORARIO_ALVO = os.environ.get("HORARIO_ALVO")

print(f"🚀 AVVIO MATRIX SERVER SHORTS (IT) PER: {HORARIO_ALVO}")
...
PASTA_TEMP = "/tmp/fabrica_shorts_it"
...
def baixar_arquivo(file_id, destino):
    for tentativa in range(4):
        try:
            ...
            print(f"   ⚠️ Google Drive fallito. Nuovo tentativo... ({tentativa+1}/4)")
            time.sleep(5)
    raise Exception(f"Fallimento definitivo download {file_id}")
    ...
    while True:
        for tentativa in range(4):
            try:
                ...
                print(f"   ⚠️ Errore cartella {folder_id}. Nuovo tentativo... ({tentativa+1}/4)")
                time.sleep(5)
        else: return res
...
for index, linha in enumerate(dados, start=2):
    if str(linha.get('Status', '')).strip() == 'Ready for Audio' and str(linha.get('Horario', '')).strip() == HORARIO_ALVO:
        ...
        print(f"🎬 AVVIO SHORT: Riga {index} - {persona} alle {horario_str}")

        voz_escolhida = "it-IT-ElsaNeural"

        print("   📥 Download immagini verticali (Maria Universal)...")
        ...
        print(f"   🎙️ Generazione voce neurale e sottotitoli ({voz_escolhida})...")
        ...
        print("   ✂️ Taglio del silenzio finale per il loop...")
        ...
        print("   🎞️ Assemblaggio blocchi visivi verticali (1080x1920)...")
        ...
        print("   🔥 Mixaggio audio e finalizzazione del Short...")
        ...
        tags_limpas = re.sub(r'[^a-zA-ZÀ-ÿ0-9 ,]', '', tags_str)
        tags_lista = [t.strip()[:30] for t in tags_limpas.split(',') if t.strip()][:15]

        texto_convite = f"\n\n🔴 IN DIRETTA — 24H CON LA MADONNA 🙏 Unisciti a noi: https://youtube.com/channel/UCa1_Xd4tOUd6GSPNu7auY4A/live\n\nPer la preghiera completa e profonda, visita il nostro canale. Pubblichiamo preghiere potenti ogni giorno."

        try:
            agora_rome = datetime.datetime.now(pytz.timezone('Europe/Rome'))
            data_hora_alvo = pytz.timezone('Europe/Rome').localize(datetime.datetime.strptime(f"{data_str} {horario_str}", "%Y-%m-%d %H:%M"))
            publish_at = data_hora_alvo.isoformat() if data_hora_alvo > agora_rome else None
        except: publish_at = None

        body = {
            "snippet": {"title": titulo[:100], "description": f"{descricao_ia}{texto_convite}", "tags": tags_lista, "categoryId": "22", "defaultLanguage": "it-IT", "defaultAudioLanguage": "it-IT"},
        ...
        for tentativa in range(3):
            try:
                ...
                print(f"   🎉 SUCCESSO! Short {video_id} pubblicato.")
                try:
                ...
                except Exception as e: print(f"   ⚠️ Errore aggiornamento: {e}")
                break
            except Exception as e:
                print(f"   ❌ Errore YouTube (Tentativo {tentativa+1}/3): {e}")
                time.sleep(15)

print("\n🚀 MATRIX SERVER SHORTS IT ARRESTATO.")
