...
print(f"🚀 AVVIO MATRIX SERVER IT PER: {HORARIO_ALVO}")
...
def _gerar_overlay_particulas(saida, largura=1920, altura=1080, duracao_s=20, fps=30):
    ...
    try:
        ...
        if proc.returncode != 0: raise RuntimeError("FFmpeg particelle fallito")
        os.rename(tmp, saida); return saida
    except Exception as e:
        try: proc.stdin.close()
        except: pass
        proc.kill()
        if os.path.exists(tmp): os.remove(tmp)
        print(f"[WARN] Particelle: {e}"); return None
...
def baixar_arquivo(file_id, destino):
    for tentativa in range(4):
        try:
            ...
            print(f"   ⚠️ Drive fallito. Nuovo tentativo... ({tentativa+1}/4)")
            time.sleep(5)
...
def criar_thumbnail(img_path, texto_curto, horario, persona, caminho_saida):
    ...
    # Italian flag colors: green morning, red evening
    cor_barra = "#009246" if "06:00" in horario else "#CE2B37"
...
for index, linha in enumerate(dados, start=2):
    if str(linha.get('Status', '')).strip() == 'Ready for Audio' and str(linha.get('Idioma', '')).strip().upper() == 'IT' and str(linha.get('Horario', '')).strip() == HORARIO_ALVO:
        ...
        print(f"   🎙️ Generazione voce neurale ({voz_escolhida} a {param_rate})...")
        ...
        print("   🎞️ Costruzione blocchi visivi...")
        ...
        print("   🔥 Mixaggio audio e finalizzazione video...")
        ...
        try:
            ...
            data_hora_alvo = tz_rome.localize(datetime.datetime.strptime(f"{data_str} {horario_str}", "%Y-%m-%d %H:%M"))
            agora_rome = datetime.datetime.now(tz_rome)
            publish_at = data_hora_alvo.isoformat() if data_hora_alvo > agora_rome else None
        except: publish_at = None

        body = {
            "snippet": {"title": titulo[:100], "description": f"{descricao_ia}{capitulos}{bloco_live}", "tags": tags_lista, "categoryId": "22", "defaultLanguage": "it-IT", "defaultAudioLanguage": "it-IT"},
